...
public class EnderChickenConfig {
	...
	public static void setId(FMLPreInitializationEvent event) {
		...
		configFile = new File(path, "EnderChicken.cfg");
		config = new Configuration(configFile);

		id = config.get("entity data", "ID", 514).getInt(514);
		config.save();
	}
	...
	public static void entityData()	{
		...
		health = config.get("entity data", "Health", 20.0D).getDouble(20.0D);
		...
		canTeleport = config.get("entity data", "Allow Teleporting", true).getBoolean(true);
		...
		damageByWater = config.get("entity data", "Damage From Water", 1.0D).getDouble(1.0F);
		emitsParticles = config.get("entity data", "Emits Particles", true).getBoolean(true);
		particleType = config.get("entity data", "Particle Type", "portal").getString();
	...
	public static void taming() {
		...
		tameItem = config.get("taming", "Taming Item", "minecraft:ender_pearl").getString();
	...
	public static void tempting() {
		...
		temptItem = config.get("tempting", "Tempting Item", "minecraft:ender_eye").getString();
	...
	public static void breeding() {	
		...
		breedItem = config.get("breeding", "Breeding Item", "minecraft:ender_pearl").getString();
	...
	public static void laying() {
		...
		layItem = config.get("laying", "Laying Item", "minecraft:ender_pearl").getString();
		...
		varItemLayTime = config.get("laying", "Variable Item Lay Time", 6000).getInt(6000);
	...
	public static void attackData() {
		...
		attackDamage = (float) config.get("attack data", "Attack Damage", 6.0D).getDouble(6.0D);
		...
		attackSpeed = config.get("attack data", "Attack Speed", 1.2599999809265137D).getDouble(1.2599999809265137D);
