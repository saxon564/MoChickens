...
public class EntityLapisChicken extends EntityMoChicken
{
   ...
   // public int timeUntilNextEgg;
    ...
    public EntityLapisChicken(World par1World)
    {
        super(par1World);
    ...
    {
        super.applyEntityAttributes(LapisChickenConfig.config, this.getClass());
	...
	public void setTamed(boolean par1) {
		...
		if (par1) {
			...
			this.tasks.addTask(3, new ChickAITempt(this, 1.0D,
					Item.getItemFromBlock((Block) Block.blockRegistry.getObject("lapis_block")), false));
			if (MoChickens.lapisTamedDespawn == false) {
				despawn = false;
				ran = 0;
				this.canDespawn();
			} else {
			...
			if (MoChickens.lapisUntamedDespawn == false) {
				despawn = false;
				ran = 0;
				this.canDespawn();
			} else {
   ...
   /* public void onLivingUpdate()
    {
        ...
        //if (loc == 0) {
        //	MinecraftServer.getServer().getLogAgent().logInfo("Lapis X:" + this.posX + " Y:" + this.posY + " Z:" + this.posZ);
        //	loc ++;
        //}
                ...
                && --this.timeUntilNextEgg == 0)
        {
            ...
            this.entityDropItem(new ItemStack((Item) Item.itemRegistry.getObject("dye"), 1, 4), 1);
    ...
    /*public boolean interact(EntityPlayer par1EntityPlayer)
    {
        ...
        if (!this.isTamed())
        {
            if (itemstack != null && itemstack.getDisplayName().toLowerCase().equalsIgnoreCase("Lapis Lazuli"))
            {
     ...
     */
    /*public EntityLapisChicken spawnBabyAnimal(EntityAgeable par1EntityAgeable)
    {
        EntityLapisChicken entitybluechicken = new EntityLapisChicken(
            this.worldObj);
        EntityLivingBase s = this.getOwner();
    ...
    /*public boolean isBreedingItem(ItemStack par1ItemStack)
    {
        if (this.isTamed())
        {
            return par1ItemStack != null
                   && par1ItemStack.getDisplayName().toLowerCase().equalsIgnoreCase("Lapis Lazuli");
        }
        else
        {
            return false;
