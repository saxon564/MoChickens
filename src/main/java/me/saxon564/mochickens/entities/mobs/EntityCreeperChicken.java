...
import me.saxon564.mochickens.entities.mobs.ai.ChickAISwell;
...
public class EntityCreeperChicken extends EntityMoChicken
{
	...
	public void setTamed(boolean par1) {
		...
		if (par1) {
			...
			this.tasks.addTask(3, new ChickAITempt(this, 1.0D,
					Item.getItemFromBlock((Block) Block.blockRegistry.getObject("tnt")), false));
			if (MoChickens.creeperTamedDespawn == false) {
				despawn = false;
				ran = 0;
				this.canDespawn();
			} else {
			...
			if (MoChickens.creeperUntamedDespawn == false) {
				despawn = false;
				ran = 0;
				this.canDespawn();
			} else {
    ...
    public void onLivingUpdate()
    {
        ...
        if (loc == 0) {
        	//System.out.println("Creeper X:" + this.posX + " Y:" + this.posY + " Z:" + this.posZ);
    ...
    /*    if (!this.isTamed())
            ...
            if (this.isEntityAlive())
            {
                ...
                if (this.timeSinceIgnited >= this.fuseTime)
                {
                    ...
                    if (!this.worldObj.isRemote)
                    {
                        ...
                        entityplayer.addPotionEffect(new PotionEffect(Potion.confusion.id, 300, 5));
                        }
                        this.setDead();
                    }
                }
                ...
                && --this.timeUntilNextEgg == 0)
        {
            ...
            this.dropItem((Item) Item.itemRegistry.getObject("gunpowder"), 1);
    ...
    /*public boolean interact(EntityPlayer par1EntityPlayer)
    {
        ...
        if (!this.isTamed())
        {
            if (itemstack != null && itemstack.getItem() == (Item) Item.itemRegistry.getObject("gunpowder"))
            {
     ...
     */
    /*public EntityCreeperChicken spawnBabyAnimal(EntityAgeable par1EntityAgeable)
    {
        EntityCreeperChicken entitybluechicken = new EntityCreeperChicken(
            this.worldObj);
        EntityLivingBase s = this.getOwner();
    ...
    /*public boolean isBreedingItem(ItemStack par1ItemStack)
    {
        if (this.isTamed())
        {
            return par1ItemStack != null
                   && par1ItemStack.getItem() == (Item) Item.itemRegistry.getObject("gunpowder");
        }
        else
        {
            return false;
