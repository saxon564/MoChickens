...
public class EntityClayChicken extends EntityMoChicken
{
   /* public boolean field_70885_d = false;
	...
	public void setTamed(boolean par1) {
		...
		if (par1) {
			...
			this.tasks.addTask(3, new ChickAITempt(this, 1.0D,
					Item.getItemFromBlock((Block) Block.blockRegistry.getObject("clay")), false));
			if (MoChickens.clayTamedDespawn == false) {
				despawn = false;
				ran = 0;
				this.canDespawn();
			} else {
			...
			if (MoChickens.clayUntamedDespawn == false) {
				despawn = false;
				ran = 0;
				this.canDespawn();
			} else {
    ...
    /*public void onLivingUpdate()
    {
        ...
        //if (loc == 0) {
        //	MinecraftServer.getServer().getLogAgent().logInfo("Clay X:" + this.posX + " Y:" + this.posY + " Z:" + this.posZ);
        //	loc ++;
        //}
                ...
                && --this.timeUntilNextEgg == 0)
        {
            ...
            this.dropItem((Item) Item.itemRegistry.getObject("clay_ball"), 1);
            this.timeUntilNextEgg = this.rand.nextInt(2000) + 6000;
        }
    }
    ...
    /*public boolean interact(EntityPlayer par1EntityPlayer)
    {
        ...
        if (!this.isTamed())
        {
            if (itemstack != null && itemstack.getItem() == (Item) Item.itemRegistry.getObject("clay_ball"))
            {
     ...
     */
    /*public EntityClayChicken spawnBabyAnimal(EntityAgeable par1EntityAgeable)
    {
        EntityClayChicken entitybluechicken = new EntityClayChicken(
            this.worldObj);
        EntityLivingBase s = this.getOwner();
    ...
    /*public boolean isBreedingItem(ItemStack par1ItemStack)
    {
        if (this.isTamed())
        {
            return par1ItemStack != null
                   && par1ItemStack.getItem() == (Item) Item.itemRegistry.getObject("clay_ball");
        }
        else
        {
            return false;
