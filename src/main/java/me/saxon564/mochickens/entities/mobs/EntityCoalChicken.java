...
public class EntityCoalChicken extends EntityMoChicken {
	...
	public void applyEntityAttributes() {
		...
		/*this.getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(CoalChickenConfig.coalHealth);
		this.getEntityAttribute(SharedMonsterAttributes.movementSpeed).setBaseValue(CoalChickenConfig.coalSpeed);*/
	}
	...
	protected boolean canDespawn() {
		if (ran == 0) {
			if (MoChickens.debug){
			  MinecraftServer.getServer().logInfo("Coal Deaspawning is: " + despawn);
			}
			ran = 1;
		}
		return despawn;
	...
	public void setTamed(boolean par1) {
		...
		if (par1) {
			...
			this.tasks.addTask(3, new ChickAITempt(this, 1.0D,
					Item.getItemFromBlock((Block) Block.blockRegistry.getObject("coal_block")), false));
			if (CoalChickenConfig.coalTamedDespawn == false) {
				despawn = false;
				ran = 0;
				this.canDespawn();
			} else {
			...
			if (CoalChickenConfig.coalUntamedDespawn == false) {
				despawn = false;
				ran = 0;
				this.canDespawn();
			} else {
	...
	/*public void onLivingUpdate() {
		...
		// if (loc == 0) {
		// MinecraftServer.getServer().getLogAgent().logInfo("Coal X:" +
		...
		if (!this.isChild() && !this.worldObj.isRemote
			...
			this.dropItem((Item) Item.itemRegistry.getObject("coal"), 1);
			this.timeUntilNextEgg = this.rand.nextInt(2000) + 6000;
		}
	}
	...
	/*public boolean interact(EntityPlayer par1EntityPlayer) {
		...
		if (!this.isTamed()) {
			if (itemstack != null && itemstack.getItem() == Item.itemRegistry.getObject("coal")) {
	 ...
	 */
	/*public EntityCoalChicken spawnBabyAnimal(EntityAgeable par1EntityAgeable) {
		EntityCoalChicken entitybluechicken = new EntityCoalChicken(
				this.worldObj);
		EntityLivingBase s = this.getOwner();
	...
	/*public boolean isBreedingItem(ItemStack par1ItemStack) {
		if (this.isTamed()) {
			return par1ItemStack != null
					&& par1ItemStack.getItem() == Item.itemRegistry.getObject("coal");
		} else {
			return false;
		}
	}
