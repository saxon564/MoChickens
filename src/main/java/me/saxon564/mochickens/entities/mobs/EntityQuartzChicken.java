...
public class EntityQuartzChicken extends EntityMoChicken
{
	...
	public void setTamed(boolean par1) {
		...
		if (par1) {
			...
			this.tasks.addTask(3, new ChickAITempt(this, 1.0D,
					Item.getItemFromBlock((Block) Block.blockRegistry.getObject("quartz_block")), false));
			if (MoChickens.quartzTamedDespawn == false) {
				despawn = false;
				ran = 0;
				this.canDespawn();
			} else {
			...
			if (MoChickens.quartzUntamedDespawn == false) {
				despawn = false;
				ran = 0;
				this.canDespawn();
			} else {
    ...
    /*public void onLivingUpdate()
    {
        ...
        //if (loc == 0) {
        //	MinecraftServer.getServer().getLogAgent().logInfo("Quartz X:" + this.posX + " Y:" + this.posY + " Z:" + this.posZ);
        //	loc ++;
        //}
                ...
                && --this.timeUntilNextEgg == 0)
        {
            ...
            this.dropItem((Item) Item.itemRegistry.getObject("quartz"), 1);
    ...
    /*public boolean interact(EntityPlayer par1EntityPlayer)
    {
        ...
        if (!this.isTamed())
        {
            if (itemstack != null && itemstack.getItem() == (Item) Item.itemRegistry.getObject("quartz"))
            {
     ...
     */
    /*public EntityQuartzChicken spawnBabyAnimal(EntityAgeable par1EntityAgeable)
    {
        EntityQuartzChicken entitybluechicken = new EntityQuartzChicken(
            this.worldObj);
        EntityLivingBase s = this.getOwner();
    ...
    /*public boolean isBreedingItem(ItemStack par1ItemStack)
    {
        if (this.isTamed())
        {
            return par1ItemStack != null
                   && par1ItemStack.getItem() == (Item) Item.itemRegistry.getObject("quartz");
        }
        else
        {
            return false;
