...
public class EntityBeefyChicken extends EntityMoChicken
{
	...
	public void setTamed(boolean par1) {
		...
		if (par1) {
			...
			this.tasks.addTask(3, new ChickAITempt(this, 1.0D,
					(Item) Item.itemRegistry.getObject("cooked_beef"), false, BeefyChickenConfig.followDelay));
			if (BeefyChickenConfig.tamedDespawn == false) {
				despawn = false;
				ran = 0;
				this.canDespawn();
			} else {
			...
			if (BeefyChickenConfig.untamedDespawn == false) {
				despawn = false;
				ran = 0;
				this.canDespawn();
			} else {
    ...
    /*public void onLivingUpdate()
    {
                ...
                && --this.timeUntilNextEgg == 0)
        {
            ...
            this.dropItem((Item) Item.itemRegistry.getObject("cooked_beef"), 1);
            this.timeUntilNextEgg = this.rand.nextInt(2500) + 6000;
        }
    }
    ...
    /*public boolean interact(EntityPlayer par1EntityPlayer)
    {
        ...
        if (!this.isTamed())
        {
            if (itemstack != null && itemstack.getItem() == (Item) Item.itemRegistry.getObject("beef"))
            {
     ...
     */
    /*public EntityBeefyChicken spawnBabyAnimal(EntityAgeable par1EntityAgeable)
    {
        EntityBeefyChicken entitybluechicken = new EntityBeefyChicken(
            this.worldObj);
        EntityLivingBase s = this.getOwner();
    ...
    /*public boolean isBreedingItem(ItemStack par1ItemStack)
    {
        if (this.isTamed())
        {
            return par1ItemStack != null
                   && par1ItemStack.getItem() == (Item) Item.itemRegistry.getObject("cooked_beef");
        }
        else
        {
            return false;
