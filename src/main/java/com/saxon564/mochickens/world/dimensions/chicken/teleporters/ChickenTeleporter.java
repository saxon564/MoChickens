...
public class ChickenTeleporter extends Teleporter {
            ...
            for (int l = -2; l <= 2; ++l)
            {
                for (int i1 = -2; i1 <= 2; ++i1)
                {
                    for (int j1 = -1; j1 < 3; ++j1)
                    {
                        ...
                        int i2 = k + i1 * b1 - l * b0;
                        boolean flag = j1 < 0;
                        this.worldServerInstance.setBlockState(new BlockPos(k1, l1, i2), flag ? MoChickens.blockFeatherBlock.getDefaultState() : Blocks.air.getDefaultState());
                    }
                }
            }
	...
	public boolean placeInExistingPortal(Entity entityIn, float p_180620_2_)
    {
        ...
        {
            ChickenTeleporter.PortalPosition portalposition = (ChickenTeleporter.PortalPosition)this.destinationCoordinateCache.getValueByKey(k);
            d0 = 0.0D;
        ...
        if (d0 >= 0.0D)
        {
            if (flag1)
            {
                this.destinationCoordinateCache.add(k, new ChickenTeleporter.PortalPosition((BlockPos)object, this.worldServerInstance.getTotalWorldTime()));
            ...
            for (i3 = -1; i3 <= 1; ++i3)
            {
                for (j3 = 1; j3 < 3; ++j3)
                {
                    for (k3 = -1; k3 < 3; ++k3)
                    {
                        ...
                        j4 = k2 + (j3 - 1) * l2 - i3 * l5;
                        boolean flag = k3 < 0;
                        this.worldServerInstance.setBlockState(new BlockPos(l3, i4, j4), flag ? MoChickens.blockFeatherBlock.getDefaultState() : Blocks.air.getDefaultState());
                    }
                }
            }
        }
        ...
        for (j3 = 0; j3 < 4; ++j3)
        {
            for (k3 = 0; k3 < 4; ++k3)
            {
                for (l3 = -1; l3 < 4; ++l3)
                {
                    ...
                    this.worldServerInstance.setBlockState(new BlockPos(i4, j4, k4), flag1 ? MoChickens.blockFeatherBlock.getDefaultState() : iblockstate, 2);
                }
            }
