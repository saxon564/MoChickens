package com.saxon564.mochickens.world.dimensions.chicken.teleporters;
...
public class ChickenTeleporter extends Teleporter {
	...
	private final Long2ObjectMap<ChickenTeleporter.PortalPosition> destinationCoordinateCache = new Long2ObjectOpenHashMap(4096);
            ...
            for (int l = -2; l <= 2; ++l)
            {
                for (int i1 = -2; i1 <= 2; ++i1)
                {
                    for (int j1 = -1; j1 < 3; ++j1)
                    {
                        ...
                        int i2 = k + i1 * b1 - l * b0;
                        boolean flag = j1 < 0;
                        this.worldServerInstance.setBlockState(new BlockPos(k1, l1, i2), flag ? MoChickens.feather_block.getDefaultState() : Blocks.AIR.getDefaultState());
                    }
                }
            }
	...
	public boolean placeInExistingPortal(Entity entityIn, float p_180620_2_)
    {
        ...
        {
            ChickenTeleporter.PortalPosition portalposition = (ChickenTeleporter.PortalPosition)this.destinationCoordinateCache.get(k);
            d0 = 0.0D;
        ...
        if (d0 >= 0.0D)
        {
            if (flag1)
            {
                this.destinationCoordinateCache.put(k, new ChickenTeleporter.PortalPosition((BlockPos)object, this.worldServerInstance.getTotalWorldTime()));
            }
            ...
            EnumFacing enumfacing1 = EnumFacing.getHorizontal(entityIn.getTeleportDirection().getIndex());
                            ...
                            for (i4 = 0; i4 < 3; ++i4)
                            {
                                for (j4 = 0; j4 < 4; ++j4)
                                {
                                    for (k4 = -1; k4 < 4; ++k4)
                                    {
                                        ...
                                        if (k4 < 0 && !this.worldServerInstance.getBlockState(new BlockPos(l4, i5, j5)).getBlock().getMaterial(this.worldServerInstance.getBlockState(new BlockPos(l4, i5, j5))).isSolid() || k4 >= 0 && !this.worldServerInstance.isAirBlock(new BlockPos(l4, i5, j5)))
                                        {
                                            continue label271;
                                        }
                                    }
                            ...
                            for (j3 = l1; j3 < l1 + 2; ++j3)
                            {
                                ...
                                for (i4 = 0; i4 < 4; ++i4)
                                {
                                    for (j4 = -1; j4 < 4; ++j4)
                                    {
                                        ...
                                        if (j4 < 0 && !this.worldServerInstance.getBlockState(new BlockPos(k4, l4, i5)).getBlock().getMaterial(this.worldServerInstance.getBlockState(new BlockPos(k4, l4, i5))).isSolid() || j4 >= 0 && !this.worldServerInstance.isAirBlock(new BlockPos(k4, l4, i5)))
                                        {
                                            continue label219;
                                        }
                                    }
            ...
            for (i3 = -1; i3 <= 1; ++i3)
            {
                for (j3 = 1; j3 < 3; ++j3)
                {
                    for (k3 = -1; k3 < 3; ++k3)
                    {
                        ...
                        j4 = k2 + (j3 - 1) * l2 - i3 * l5;
                        boolean flag = k3 < 0;
                        this.worldServerInstance.setBlockState(new BlockPos(l3, i4, j4), flag ? MoChickens.feather_block.getDefaultState() : Blocks.AIR.getDefaultState());
                    }
                }
            }
        }
        ...
        for (j3 = 0; j3 < 4; ++j3)
        {
            for (k3 = 0; k3 < 4; ++k3)
            {
                for (l3 = -1; l3 < 4; ++l3)
                {
                    ...
                    this.worldServerInstance.setBlockState(new BlockPos(i4, j4, k4), flag1 ? MoChickens.feather_block.getDefaultState() : iblockstate, 2);
                }
            }
            ...
            for (k3 = 0; k3 < 4; ++k3)
            {
                for (l3 = -1; l3 < 4; ++l3)
                {
                    ...
                    j4 = j2 + l3;
                    k4 = k2 + (k3 - 1) * l2;
                    this.worldServerInstance.notifyNeighborsOfStateChange(new BlockPos(i4, j4, k4), this.worldServerInstance.getBlockState(new BlockPos(i4, j4, k4)).getBlock(), false);
                }
            }
        }
