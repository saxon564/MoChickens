...
public class RainbowChickenConfig {
	...
	public static int[] effectAmplifiers = {4, 2};
	...
	public static void setId(FMLPreInitializationEvent event) {
		...
		configFile = new File(path, "RainbowChicken.cfg");
		config = new Configuration(configFile);

		id = config.get("entity data", "ID", 512).getInt(512);
		config.save();
	}
	...
	public static void tempting() {
		...
		temptItem = config.get("tempting", "Tempting Item", "minecraft:wool").getString();
	...
	public static void laying() {
		...
		layItemDataRandom = config.get("laying", "Laying Item Uses Random Data", true).getBoolean(true);
		layItemMinData = config.get("laying", "Laying Item Min Data", 0).getInt(0);
		layItemMaxData = config.get("laying", "Laying Item Max Data", 15).getInt(15);
	...
	public static void attackData() {
		...
		attackDamage = (float) config.get("attack data", "Attack Damage", 4.0D).getDouble(4.0D);
