...
public class EntityEnchantedChicken extends EntityMoChicken
{
    ...
    public EntityEnchantedChicken(World par1World)
    {
        super(par1World);
    ...
    {
        super.applyEntityAttributes(EnchantedChickenConfig.config, this.getClass());
    ...
    protected boolean canDespawn() {
		if (ran == 0) {
			//System.out.println("Enchanted Deaspawning is: " + despawn);
			ran = 1;
		}
		return despawn;
	}
	...
	public void setTamed(boolean par1) {
		...
		if (par1) {
			...
			this.tasks.addTask(3, new ChickAITempt(this, 1.0D,
					Item.getItemFromBlock((Block) Block.blockRegistry.getObject("enchanting_table")), false));
			if (MoChickens.enchantedTamedDespawn == false) {
				despawn = false;
				ran = 0;
				this.canDespawn();
			} else {
			...
			if (MoChickens.enchantedUntamedDespawn == false) {
				despawn = false;
				ran = 0;
				this.canDespawn();
			} else {
    ...
    /*public void onLivingUpdate()
    {
                ...
                && --this.timeUntilNextEgg == 0)
        {
            ...
            this.dropItem((Item) Item.itemRegistry.getObject("experience_bottle"), 1);
            this.timeUntilNextEgg = this.rand.nextInt(6000) + 6000;
        }
    }
    
    private void addLight() {
    	this.worldObj.setLightValue(EnumSkyBlock.Block, (int)this.posX, (int)this.posY, (int)this.posZ, 4);
    ...
    /*public boolean interact(EntityPlayer par1EntityPlayer)
    {
        ...
        if (!this.isTamed())
        {
            if (itemstack != null && itemstack.getItem() == (Item) Item.itemRegistry.getObject("enchanted_book"))
            {
     ...
     */
    /*public EntityEnchantedChicken spawnBabyAnimal(EntityAgeable par1EntityAgeable)
    {
        EntityEnchantedChicken entitybluechicken = new EntityEnchantedChicken(
            this.worldObj);
        EntityLivingBase s = this.getOwner();
    ...
    /*public boolean isBreedingItem(ItemStack par1ItemStack)
    {
        if (this.isTamed())
        {
            return par1ItemStack != null
                   && par1ItemStack.getItem() == (Item) Item.itemRegistry.getObject("experience_bottle");
        }
        else
        {
            return false;
