...
import me.saxon564.mochickens.entities.mobs.ai.ChickAITemptDye;
...
public class EntityCookieChicken extends EntityMoChicken
{
    ...
    public EntityCookieChicken(World par1World)
    {
        super(par1World);
	...
	public void setTamed(boolean par1) {
		...
		if (par1) {
			...
			this.tasks.addTask(3, new ChickAITemptDye(this, 1.0D,
                    new ItemStack((Item) Item.itemRegistry.getObject("dye"), 1, 3).getDisplayName()
                    .toLowerCase(), false));
			if (MoChickens.cookieTamedDespawn == false) {
				despawn = false;
				ran = 0;
				this.canDespawn();
			} else {
			...
			if (MoChickens.cookieUntamedDespawn == false) {
				despawn = false;
				ran = 0;
				this.canDespawn();
			} else {
        ...
        if (!this.isTamed())
        {
            ...
            if (entityplayer instanceof EntityLivingBase)
            {
                ...
                entityplayer.addPotionEffect(new PotionEffect(
                                                 Potion.moveSpeed.id, 200, 3));
                entityplayer.addPotionEffect(new PotionEffect(
                                                 Potion.confusion.id, 200, 3));
            }
   ...
   /*public void onLivingUpdate()
    {
        ...
        //if (loc == 0) {
        //	MinecraftServer.getServer().getLogAgent().logInfo("Cookie X:" + this.posX + " Y:" + this.posY + " Z:" + this.posZ);
        //	loc ++;
        //}
                ...
                && --this.timeUntilNextEgg == 0)
        {
            ...
            this.dropItem((Item) Item.itemRegistry.getObject("cookie"), 1);
            this.timeUntilNextEgg = this.rand.nextInt(2500) + 6000;
        }
    }
    ...
    /*protected void dropFewItems(boolean par1, int par2)
    {
        ...
        for (int k = 0; k < j; ++k)
        {
            ...
            {
                this.dropItem((Item) Item.itemRegistry.getObject("sugar"), 1);
            }
        }
    ...
    /*public boolean interact(EntityPlayer par1EntityPlayer)
    {
        ...
        if (!this.isTamed())
        {
            if (itemstack != null && itemstack.getItem() == (Item) Item.itemRegistry.getObject("cookie"))
            {
     ...
     */
    /*public EntityCookieChicken spawnBabyAnimal(EntityAgeable par1EntityAgeable)
    {
        EntityCookieChicken entitybluechicken = new EntityCookieChicken(
            this.worldObj);
        EntityLivingBase s = this.getOwner();
