...
public class EntityRainbowChicken extends EntityMoChicken
{
    ...
    public EntityRainbowChicken(World par1World)
    {
        super(par1World);
	...
	public void setTamed(boolean par1) {
		...
		if (par1) {
			...
			this.tasks.addTask(3, new ChickAITempt(this, 1.0D,
					Item.getItemFromBlock((Block) Block.blockRegistry.getObject("wool")), false));
			if (MoChickens.rainbowTamedDespawn == false) {
				despawn = false;
				ran = 0;
				this.canDespawn();
			} else {
			...
			if (MoChickens.rainbowUntamedDespawn == false) {
				despawn = false;
				ran = 0;
				this.canDespawn();
			} else {
        ...
        if (!this.isTamed())
        {
            ...
            if (entityplayer instanceof EntityLivingBase)
            {
                ...
                entityplayer.addPotionEffect(new PotionEffect(
                                                 Potion.confusion.id, 200, 4));
                entityplayer.addPotionEffect(new PotionEffect(
                                                 Potion.blindness.id, 200, 2));
            }
    ...
    /*public void onLivingUpdate()
    {
        ...
        //if (loc == 0) {
        //	MinecraftServer.getServer().getLogAgent().logInfo("Rainbow X:" + this.posX + " Y:" + this.posY + " Z:" + this.posZ);
        //	loc ++;
        //}
                ...
                && --this.timeUntilNextEgg == 0)
        {
            ...
            this.entityDropItem(new ItemStack((Item) Item.itemRegistry.getObject("dye"), 1, this.randomInt(0, 15)), 1);
            this.timeUntilNextEgg = this.rand.nextInt(2000) + 6000;
        }
    }
    ...
    /*public boolean interact(EntityPlayer par1EntityPlayer)
    {
        ...
        if (!this.isTamed())
        {
            if (itemstack != null && itemstack.getItem() == (Item) Item.itemRegistry.getObject("dye"))
            {
     ...
     */
    /*public EntityRainbowChicken spawnBabyAnimal(EntityAgeable par1EntityAgeable)
    {
        EntityRainbowChicken entitybluechicken = new EntityRainbowChicken(
            this.worldObj);
        EntityLivingBase s = this.getOwner();
    ...
    /*public boolean isBreedingItem(ItemStack par1ItemStack)
    {
        if (this.isTamed())
        {
            return par1ItemStack != null
                   && par1ItemStack.getItem() == (Item) Item.itemRegistry.getObject("dye");
        }
        else
        {
            return false;
