...
public class EntitySnowChicken extends EntityMoChicken
{
	...
	public void setTamed(boolean par1) {
		...
		if (par1) {
			...
			this.tasks.addTask(3, new ChickAITempt(this, 1.0D,
					Item.getItemFromBlock((Block) Block.blockRegistry.getObject("snow")), false));
			if (MoChickens.snowTamedDespawn == false) {
				despawn = false;
				ran = 0;
				this.canDespawn();
			} else {
			...
			if (MoChickens.snowUntamedDespawn == false) {
				despawn = false;
				ran = 0;
				this.canDespawn();
			} else {
    ...
    /*public void onLivingUpdate()
    {
        ...
        //if (loc == 0)
        //{
        //    MinecraftServer.getServer().getLogAgent().logInfo("Snow X:" + this.posX + " Y:" + this.posY + " Z:" + this.posZ);
        //    loc ++;
        //}
                ...
                && --this.timeUntilNextEgg == 0)
        {
            ...
            this.dropItem((Item) Item.itemRegistry.getObject("snowball"), 1);
            this.timeUntilNextEgg = this.rand.nextInt(1000) + 6000;
        }
    }
    ...
    /*public boolean interact(EntityPlayer par1EntityPlayer)
    {
        ...
        if (!this.isTamed())
        {
            if (itemstack != null && itemstack.getItem() == (Item) Item.itemRegistry.getObject("snowball"))
            {
     ...
     */
    /*public EntitySnowChicken spawnBabyAnimal(EntityAgeable par1EntityAgeable)
    {
        EntitySnowChicken entitybluechicken = new EntitySnowChicken(
            this.worldObj);
        EntityLivingBase s = this.getOwner();
    ...
    /*public boolean isBreedingItem(ItemStack par1ItemStack)
    {
        if (this.isTamed())
        {
            return par1ItemStack != null
                   && par1ItemStack.getItem() == (Item) Item.itemRegistry.getObject("snowball");
        }
        else
        {
            return false;
