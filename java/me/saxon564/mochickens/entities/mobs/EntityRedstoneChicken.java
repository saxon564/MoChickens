...
public class EntityRedstoneChicken extends EntityMoChicken
{
    ...
    {
        super.applyEntityAttributes(RedstoneChickenConfig.config, this.getClass());
	...
	public void setTamed(boolean par1) {
		...
		if (par1) {
			...
			this.tasks.addTask(3, new ChickAITempt(this, 1.0D,
					Item.getItemFromBlock((Block) Block.blockRegistry.getObject("redstone_block")), false));
			if (MoChickens.redstoneTamedDespawn == false) {
				despawn = false;
				ran = 0;
				this.canDespawn();
			} else {
			...
			if (MoChickens.redstoneUntamedDespawn == false) {
				despawn = false;
				ran = 0;
				this.canDespawn();
			} else {
     ...
     * private void addSignal() {
    	...
    	try {
			BlockCompressedPowered.class.newInstance().isProvidingWeakPower(this.worldObj, (int)this.posX, (int)this.posY, (int)this.posZ, 15);;
			this.worldObj.markBlockRangeForRenderUpdate((int)this.posX, (int)this.posY, (int)this.posX, 12, 12, 12);
			this.worldObj.getBlockPowerInput((int)this.posX, (int)this.posY, (int)this.posZ);
    ...
    */
    ...
    /*public void onLivingUpdate()
    {
        ...
        //if (loc == 0) {
        //	MinecraftServer.getServer().getLogAgent().logInfo("Redstone X:" + this.posX + " Y:" + this.posY + " Z:" + this.posZ);
        //	loc ++;
        //}
                ...
                && --this.timeUntilNextEgg == 0)
        {
            ...
            this.dropItem((Item) Item.itemRegistry.getObject("redstone"), 1);
            this.timeUntilNextEgg = this.rand.nextInt(2000) + 6000;
        }
    }
    ...
    /*public boolean interact(EntityPlayer par1EntityPlayer)
    {
        ...
        if (!this.isTamed())
        {
            if (itemstack != null && itemstack.getItem() == (Item) Item.itemRegistry.getObject("redstone"))
            {
     ...
     */
    /*public EntityRedstoneChicken spawnBabyAnimal(EntityAgeable par1EntityAgeable)
    {
        EntityRedstoneChicken entitybluechicken = new EntityRedstoneChicken(
            this.worldObj);
        EntityLivingBase s = this.getOwner();
    ...
    /*public boolean isBreedingItem(ItemStack par1ItemStack)
    {
        if (this.isTamed())
        {
            return par1ItemStack != null
                   && par1ItemStack.getItem() == (Item) Item.itemRegistry.getObject("redstone");
        }
        else
        {
            return false;
