...
public class EntityNuuwChicken extends EntityMoChicken
{
    ...
    public EntityNuuwChicken(World par1World)
    {
        super(par1World);
	...
	public void setTamed(boolean par1) {
		...
		if (par1) {
			...
			this.tasks.addTask(3, new ChickAITempt(this, 1.0D,
					(Item) Item.getItemFromBlock((Block) Block.blockRegistry.getObject("grass")), false));
			
			if (MoChickens.nuuwTamedDespawn == false) {
				despawn = false;
				ran = 0;
				this.canDespawn();
			} else {
			...
			if (MoChickens.nuuwUntamedDespawn == false) {
				despawn = false;
				ran = 0;
				this.canDespawn();
			} else {
    ...
    /*public void onLivingUpdate()
    {
        ...
        //if (loc == 0) {
        //	MinecraftServer.getServer().getLogAgent().logInfo("Nuuw X:" + this.posX + " Y:" + this.posY + " Z:" + this.posZ);
        //	loc ++;
        //}
                ...
                && --this.timeUntilNextEgg == 0)
        {
            ...
            this.dropItem((Item) Item.getItemFromBlock((Block) Block.blockRegistry.getObject("dirt")), 1);
            this.timeUntilNextEgg = this.rand.nextInt(2000) + 6000;
        }
    }
     ...
     */
    /*public EntityNuuwChicken spawnBabyAnimal(EntityAgeable par1EntityAgeable)
    {
        EntityNuuwChicken entitybluechicken = new EntityNuuwChicken(
            this.worldObj);
        EntityLivingBase s = this.getOwner();
    ...
    /*public boolean isBreedingItem(ItemStack par1ItemStack)
    {
        if (this.isTamed())
        {
            return par1ItemStack != null
                   && par1ItemStack.getItem() == (Item) Item.getItemFromBlock((Block) Block.blockRegistry.getObject("grass"));
        }
        else
        {
            return false;
